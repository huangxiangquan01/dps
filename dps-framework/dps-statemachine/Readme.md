...
1. 分层状态机结构，以简化复杂的状态配置。
1. 状态机区域提供更复杂的状态配置。
1. 使用触发器，转换，警卫和操作。
1. 键入安全配置适配器。
1. 生成器模式，用于在Spring Application上下文之外使用的简单实例化通常用例的食谱
1. 基于Zookeeper的分布式状态机
1. 状态机事件监听器。
1. UML Eclipse Papyrus建模。
1. 将计算机配置存储在永久存储中。
1. Spring IOC集成将bean与状态机关联起来。
...
@Configuration
    ...
    public class Persist<E, S> {
         ...
         * @return:
         */
        @Bean(name = "stateMachineMemPersist")
